...
describe( 'selectors', () => {
	...
	describe( '#areProductCategoriesLoaded', () => {
		test( 'should be false when state is not available.', () => {
			expect( areProductCategoriesLoaded( preInitializedState, 1, 123 ) ).to.be.false;
		} );

		test( 'should be false when categories are currently being fetched.', () => {
			expect( areProductCategoriesLoaded( loadingState, 1, 123 ) ).to.be.false;
		} );

		test( 'should be true when categories are loaded.', () => {
			expect( areProductCategoriesLoaded( loadedState, 1, 123 ) ).to.be.true;
		} );

		test( 'should be false when categories are loaded only for a different site.', () => {
			expect( areProductCategoriesLoaded( loadedState, 1, 456 ) ).to.be.false;
		} );

		test( 'should get the siteId from the UI tree if not provided.', () => {
			expect( areProductCategoriesLoaded( loadedStateWithUi, 1 ) ).to.be.true;
		} );
	} );

	describe( '#areProductCategoriesLoading', () => {
		test( 'should be false when state is not available.', () => {
			expect( areProductCategoriesLoading( preInitializedState, 1, 123 ) ).to.be.false;
		} );

		test( 'should be true when categories are currently being fetched.', () => {
			expect( areProductCategoriesLoading( loadingState, 1, 123 ) ).to.be.true;
		} );

		test( 'should be false when categories are loaded.', () => {
			expect( areProductCategoriesLoading( loadedState, 1, 123 ) ).to.be.false;
		} );

		test( 'should be false when categories are loaded only for a different site.', () => {
			expect( areProductCategoriesLoading( loadedState, 1, 456 ) ).to.be.false;
		} );

		test( 'should get the siteId from the UI tree if not provided.', () => {
			expect( areProductCategoriesLoading( loadedStateWithUi, 1 ) ).to.be.false;
		} );
	} );
